...
public class Config {
    ...
    public boolean forcePokemonSwap = false;
    ...
    public boolean maxTierSelectIsHighestStreak = false;
    ...
    public void loadConfig() throws IOException {
        ...
        newRoot.addProperty("config_version", (Number) me.plascmabue.cobblemonbattlefactory.utils.ConfigBootstrap.BUNDLED_CONFIG_VERSION);
        if (root.has("debug")) {
        ...
        if (root.has("cancel_cooldown_seconds")) {
        ...
        for (JsonElement jsonElement : bannedBagItemsArray) {
            ...
            if (!BuiltInRegistries.ITEM.containsKey(ResourceLocation.parse((String)string))) continue;
            this.bannedBagItems.add((Item)BuiltInRegistries.ITEM.get(ResourceLocation.parse((String)string)));
        }
        bannedBagItemsArray = new JsonArray();
        for (Item Items : this.bannedBagItems) {
            bannedBagItemsArray.add(BuiltInRegistries.ITEM.getKey(Items).toString());
        }
        ...
        JsonObject tiersRoot = new JsonObject();
        if (root.has("tiers")) {
            tiersRoot = root.get("tiers").getAsJsonObject();
        }
        for (String key : tiersRoot.keySet()) {
            JsonArray possibleRentalPoolsArray;
            JsonObject tierObject = tiersRoot.get(key).getAsJsonObject();
            ...
            for (int i = 1; i <= battlesForNextTier; ++i) {
                ...
                Object world = BattleFactory.INSTANCE.server().overworld();
                if (battleLocationObject.has("world")) {
                    String worldPath = battleLocationObject.get("world").getAsString();
                    for (ServerLevel w : BattleFactory.INSTANCE.server().getAllLevels()) {
                        String id = w.dimension().location().toString();
                        String path = w.dimension().location().getPath();
                ...
                battleLocations.put(i, new Location((ServerLevel)world, playerX, playerY, playerZ, yRot, xRot, npcX, npcY, npcZ));
            }
            ...
            HashMap<Integer, DistributionSection> perRoundRewards = new HashMap<Integer, DistributionSection>();
            if (tierObject.has("per_round_rewards")) {
                JsonObject perRoundRewardsObject = tierObject.get("per_round_rewards").getAsJsonObject();
                for (String roundKey : perRoundRewardsObject.keySet()) {
                    try {
                        int roundNum = Integer.parseInt(roundKey);
                        perRoundRewards.put(roundNum, ConfigUtils.getDistributionSection(perRoundRewardsObject.get(roundKey).getAsJsonObject()));
                    } catch (NumberFormatException e) {
                        // skip invalid round key
                    }
                }
            }
            ...
            if (hasBonusEncounter) {
                ...
                if (bonusEncounterObject.has("bonus_encounter_location")) {
                    ...
                    ServerLevel world = BattleFactory.INSTANCE.server().overworld();
            ...
            this.tiers.add(new TierSettings(key, displayName, displayItem, battlesForNextTier, battleLocations, possibleNPCs, possibleRentalPools, perBattleRewards, perRoundRewards, tierCompletionRewards, hasBonusEncounter, bonusEncounterNPC, bonusEncounterLocation, bonusEncounterRewards));
        }
        newRoot.add("tiers", (JsonElement)tiersRoot);
        configFile.delete();
