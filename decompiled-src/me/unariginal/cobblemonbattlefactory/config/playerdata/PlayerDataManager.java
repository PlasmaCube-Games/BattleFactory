...
public class PlayerDataManager {
    ...
    public static PlayerData loadPlayerData(class_3222 player) {
        File playerFile;
        File playersFolder;
        File rootFolder = FabricLoader.getInstance().getConfigDir().resolve("BattleFactory").toFile();
        if (!rootFolder.exists()) {
        ...
        if ((playerFile = FabricLoader.getInstance().getConfigDir().resolve("BattleFactory/players/" + player.method_5845() + ".json").toFile()).exists()) {
            try {
                JsonObject root = JsonParser.parseReader((Reader)new FileReader(playerFile)).getAsJsonObject();
                int highestStreak = 0;
                ...
                BattleFactory.INSTANCE.playerCooldowns.put(player.method_5667(), cooldownProgress);
                return new PlayerData(player.method_5667(), player.method_5820(), highestStreak, highestCompletedTier, cooldownProgress);
            }
    ...
    public static PlayerData savePlayerData(class_3222 player) {
